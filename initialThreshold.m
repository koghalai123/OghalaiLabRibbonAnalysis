...
% 
    ...
    noThreshold=(single(data(yMin:yMax,:,:)-globalMin)/(difference));
    ...
    if isNucleus==false
        ...
        almostFilteredData(yMin:yMax,:,:)=noThreshold>threshold;
        for i = 1:size(data,3)
            
            filteredData(yMin:yMax,:,i)=medfilt2(almostFilteredData(yMin:yMax,:,i),medRange);
        end
    end
end